            ...
            <div class="col-md-8">
                <div class="card shadow-lg p-3 mb-5 bg-white rounded">
                    <div class="card-header">
                        ...
                        @endif
                        <form id="pcRegistrationForm" action="{{url('profile-post')}}" method="post">
                            {{ method_field('PUT') }}
                            {{ csrf_field() }}
                            <div class="row mb-4">
                                <div class="col">
                ...
                <div class="card shadow-lg p-3 mb-5 bg-white rounded" style="margin-top: 20px">
                    <div class="card-header">
                        <h5 class="text-center">{{ __('Slaptažodžio keitimas.') }}</h5>
                        <form id="pcRegistrationForm" action="{{route('change-password')}}" method="post">
                        {{ csrf_field() }}

                        <!-- Email input -->
                            <div class="form-outline mb-4">
                                <input type="password" id="current_password" name="current_password"  class="form-control"required />
                                <label class="form-label" for="form6Example3">Dabartinis slaptažodis</label>
                            </div>

                            <!-- Tel. Nr input -->
                            <div class="form-outline mb-4">
                                <input type="password" id="new_password" name="new_password"  class="form-control" required />
                                <label class="form-label" for="form6Example4">Naujas slaptažodis</label>
                            </div>

                            <!-- Miestas input -->
                            <div class="form-outline mb-4">
                                <input type="password" id="new_confirmed_password" name="new_confirmed_password"  class="form-control" required/>
                                <label class="form-label" for="form6Example7">Pakartoti naują slaptažodį</label>
                            </div>
                            <!-- Submit button -->
                            <button type="submit" class="btn btn-primary btn-block mb-4">Pateikti</button>
                        </form>
